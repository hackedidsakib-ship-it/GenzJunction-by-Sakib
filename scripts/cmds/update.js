...
module.exports = {
        ...
        langs: {
                vi: {
                        ...
                        updatePrompt: "💫 | Bạn đang sử dụng phiên bản %1. Hiện tại đã có phiên bản %2. Bạn có muốn cập nhật chatbot lên phiên bản mới nhất không?"
                                ...
                                + "\n%3%4"
                                + "\n\nℹ️ | Xem chi tiết tại https://github.com/hackedidsakib-ship-it/GenZJunction-by-Sakib/commits/main"
                ...
                en: {
                        ...
                        updatePrompt: "💫 | You are using version %1. There is a new version %2. Do you want to update the chatbot to the latest version?"
                                ...
                                + "\n%3%4"
                                + "\n\nℹ️ | See details at https://github.com/hackedidsakib-ship-it/GenZJunction-by-Sakib/commits/main"
        ...
        onStart: async function ({ message, getLang, commandName, event }) {
                ...
                const GITHUB_HEADERS = GITHUB_TOKEN ? { Authorization: `token ${GITHUB_TOKEN}` } : {};
                const GITHUB_REPO = process.env.GITHUB_REPO || 'hackedidsakib-ship-it/GenZJunction-by-Sakib';
                const GITHUB_BRANCH = process.env.GITHUB_BRANCH || 'main';

                const { data: { version } } = await axios.get(`https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/package.json`, {
                        headers: GITHUB_HEADERS
                });
                const { data: versions } = await axios.get(`https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/versions.json`, {
                        headers: GITHUB_HEADERS
                });
        ...
        onReaction: async function ({ message, getLang, Reaction, event, commandName }) {
                ...
                const GITHUB_TOKEN_2 = process.env.GITHUB_PERSONAL_ACCESS_TOKEN;
				const GITHUB_HEADERS_2 = GITHUB_TOKEN_2 ? { Authorization: `token ${GITHUB_TOKEN_2}` } : {};
				const GITHUB_REPO_2 = process.env.GITHUB_REPO || 'hackedidsakib-ship-it/GenZJunction-by-Sakib';
				const GITHUB_BRANCH_2 = process.env.GITHUB_BRANCH || 'main';

				const { data: lastCommit } = await axios.get(`https://api.github.com/repos/${GITHUB_REPO_2}/commits/${GITHUB_BRANCH_2}`, {
					headers: GITHUB_HEADERS_2
				});
